...
- 现有 `$apply-jobradar` 只保留浏览器确认门和最小日志骨架；在 Bonjour 侧边栏预演完成前，不得把它描述为稳定外部投递能力。
...
- 已覆盖：Skill 入口、材料采集边界、本地存储说明、结构化档案、本地硬限制、可审计 Markdown 定制稿、授权门、授权后工作台一致评分回读、首次内部飞书通知队列、Codex 安装和 WorkBuddy 启动烟测。
