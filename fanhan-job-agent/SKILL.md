...
3. 明确告诉用户这些网站信息将交给泛函团队用于补充岗位来源清单，并询问是否同意。只有“同意分享”等清晰答复才能运行 `source_feedback.py send --confirmed`；拒绝、沉默或含糊答复都不得发送。
4. 接口配置完成时只有脚本返回 `sent` 才能说已发送。返回 `pending_configuration` 时只能说已保存为本地待发送记录，不能伪装成已经进入飞书。
...
6. HTML 生成后立即把它作为本轮唯一成稿附件展示在 Codex 侧边栏，并提供可点击链接；不能只报告文件路径或先展示 PDF。用户直接修改 HTML，页面在本地自动保存文字草稿；用户亲自点击右上角“导出 PDF”后，页面必须读取当前修改并在侧边栏内生成 `.fanhan-job-agent/outbox/<同一文件名>.pdf`，不得调用系统打印窗口或跳转默认浏览器。用户尚未明确表示完成编辑和导出时，禁止查找旧 PDF、自动生成 PDF 或填写 `profile.application_resume.path`。
