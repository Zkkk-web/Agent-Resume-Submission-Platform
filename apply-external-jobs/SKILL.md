...
7. 返回 `$fanhan-job-agent` 执行“选岗后的统一定制流程”：展示匹配点、缺口、风险和具体修改建议；第一份候选人可见成稿必须是与当前公司和职位绑定的完整可编辑 HTML，并立即在 Codex 侧边栏展示。用户检查、修改并亲自导出同名 PDF 后才能继续；不得调用其他脚本直接生成 PDF，缺少任一产物时不得打开申请表。
