...
工作台已有候选人幂等入库、候选人评分存储、岗位库、岗位中心匹配、匹配审核和单岗位投递状态机，可以复用。三天版仍有三个 P0 缺口：

1. 开源、运行在候选人本地的 Skill 没有安全的工作台入库身份。现有数据库服务 Token 和候选人入库 Token 都不能随 Skill 分发。
2. 现有匹配接口以岗位为中心返回 Top N，不能保证返回“指定候选人 × 指定岗位”的评分。
3. 现有飞书通知是企业投递后的候选人状态通知，不是“候选人首次入库后发送到泛函内部话题群”。

在这三个缺口补齐前，Skill 可以完成本地档案、材料定制和外部投递，但不能安全声称已完成泛函主链路。
...
Skill 不复制这套公式。后续应由工作台增加一个认证的“指定候选人 × 指定岗位”评估接口，内部直接调用现有 `matchCandidate`；否则调用岗位 Top N 不能保证目标候选人出现在结果里。
...
1. 先补候选人自助入库身份和最小上传入口。
2. 再补指定候选人 × 指定岗位评估接口。
3. 再补首次入库通知队列和飞书发送状态。
4. Skill 才接入授权预览、幂等入库、评分读取和状态回读。
5. 本地档案、定制材料与 JobRadar 外部投递继续复用当前安全骨架。
